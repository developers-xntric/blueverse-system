...
function Hero() {
  return (
    <section id="top" className="relative h-130 overflow-hidden bg-brand-navy text-white md:h-152.25">
      ...
      />
      <div className="relative mx-auto flex h-full 2xl:max-w-360 items-center w-[90%]">
        <motion.div
        ...
        >
          ...
          <h1 className="mt-3.75 font-heading text-[35px] leading-10.5 font-bold md:text-[65px] md:leading-[72.681px]">
          ...
          {washTypes.map(([title, description]) => (
            ...
            >
              ...
              <div className="flex flex-col items-center gap-3.75">
                <h3 className="font-heading text-[20px] leading-6 md:text-[24.227px] font-bold md:leading-[33.918px] text-brand-navy">{title}</h3>
                <p className="text-[15px] leading-5 md:text-[16px] md:leading-[22.2px] text-brand-soft">{description}</p>
              </div>
            </motion.article>
          ))}
        </div>
          ...
          {businessModels.map(([icon, title, description]) => (
            ...
            >
              ...
              <div className="flex w-full flex-col items-center gap-4">
                <h3 className="font-heading text-[20px] leading-6.25 md:text-[24.227px] font-bold md:leading-[33.918px] text-brand-navy">{title}</h3>
                <p className="text-[15px] leading-5 md:text-[16px] md:leading-[22.2px] text-brand-muted">{description}</p>
              </div>
            </motion.article>
          ))}
        </div>
...
function RegionalCtas() {
  return (
    ...
    >
      ...
      />
      <div className="mx-auto grid 2xl:max-w-360 w-[90%] gap-7.5 lg:grid-cols-2">
        {regionalCards.map(([icon, region, site, copy, cta, href]) => (
          <motion.article
            key={region}
            whileHover={{ y: -5 }}
            className="flex  flex-col items-center rounded-2xl border border-white/20 bg-[rgba(255,255,255,0.1)] p-4 md:p-10 text-center hover:bg-[rgba(255,255,255,0.13)]"
          >
            <div className="flex w-full flex-1 flex-col items-center gap-7.5 ">
              <Image src={`/vehicle-assets/${icon}`} alt="" width={58.145} height={58.145} className="size-10 md:size-[58.145px]" />
              <div className="flex flex-col items-center gap-2.5">
                ...
                <p className="max-w-182.25 text-[15px] leading-5 md:text-[16.959px]">{copy}</p>
              </div>
